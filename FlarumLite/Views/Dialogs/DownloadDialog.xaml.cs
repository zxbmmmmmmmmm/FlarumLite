...
using Discussion = FlarumApi.Models.Discussion;
...
namespace FlarumLite.Views.Dialogs
{
    public sealed partial class DownloadDialog : ContentDialog
    {
        ...
        public async Task Download(int min, int range)
        {
            ...
            var posts = await FlarumApiProviders.GetPostsWithId(list, Common.Settings.Forum, Common.Settings.Token);
        ...
        public async Task SaveFile()
        {
            ...
            foreach (var post in Posts)
            {
                if ((bool)PosterCheckBox.IsChecked)
                {
                    ...
                    {
                        if (post.User.Id != 0)
                            text = text.Insert(text.Length, $"\n *** \n **[{post.User.DisplayName}](https://{Common.Settings.Forum}/u/{post.User.UserName})**  {post.CreatedAt.Value}  #{post.Number} \n\n");
                        else
                            text = text.Insert(text.Length, $"\n *** \n **已注销** {post.CreatedAt.Value} #{post.Number} \n\n");
        ...
        {
            var str = CSStoMarkdown.HTMLtoMarkdown(html);
